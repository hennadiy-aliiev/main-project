<!doctype html>
<html lang="en">
<head>
    <title>main project</title>
	<meta charset="UTF-8">
    <meta name="author" content="Hennadii Aliiev">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="shortcut icon" href="images/favicon.ico" type="image/x-icon">
    <link rel="icon" href="images/favicon.ico" type="image/x-icon">
    <link rel="stylesheet" type="text/css" href="css/style.css">
</head>
<body>
    <div class="wrapper">
       <div class="dropnav">
       <div class="dropdown">
           <button onclick="myFunction()" class="dropbtn">&equiv;</button>
           <div id="myDropdown" class="dropdown-content">
                <a href="#ab">ABOUT</a>
                <a href="#tech">TECHNOLOGIES</a>
                <a href="#feed">FEEDBACK</a>
                <a href="#cont">CONTACTS</a>
            </div>
        </div>
        </div>
        <div class="nav">
            <nav>
                <ul>
                    <li><a href="#ab">ABOUT</a></li>
                    <li><a href="#tech">TECHNOLOGIES</a></li>
                    <li><a href="#feed">FEEDBACK</a></li>
                    <li><a href="#cont">CONTACTS</a></li>
                </ul>
            </nav>
        </div>
        <div class="home">
            <div class="absol">
                <div class="name">
                    <div class="backgr">
                        HENNADII ALIIEV<br>- web developer -
                    </div>
                    <div>
                        <p>WEB SITE CREATING</p>
                    </div>
                </div>
                    <div class="for"><br>
                        for your business...<br>
                        for company...<br>
                        for sales...<br>
                        <span>for everyone.</span>
                    </div>
            </div>
            <div class="divbutton1">
                <a href="#cont">CONTACT ME</a>
            </div>
            <div class="divbutton2">
                <a href="#feed">LEAVE A FEEDBACK</a>
            </div>
        </div>
        <div class="about" id="ab">
            <h1 class="firsth1">Hello!<br><br>
            My name is Hennadii.<br>
            I am web developer.
            </h1><br>
            <div class="clearfix">
               <img class="mainfotocut" src="images/mainfotocut.jpg" alt="mainfoto">
               <p>I haven't known web developing since today.
               Several years I have been studing in Faculty of Economics.
               I have graduated from Lviv Commerce Academy in Lviv, Ukraine.<br><br>
               Sometimes I was wondering how people creating web sites.
               In my opinion it was hard, so i have learned web fundamental programming base and from now on i can tell for sure - it's far from hard but of course not easy too.<br><br>
               <br><br>
               </p>
            </div>
            <img class="mainothers" src="images/mainfoto1.jpg" alt="mainfoto1">
            <img class="mainothers" src="images/mainfoto2.jpg" alt="mainfoto2">
            <img class="mainothers" src="images/mainfoto3.jpg" alt="mainfoto3">
        </div>
        <div class="technologies" id="tech">
            <div class="fundamentals">
                <img src="images/techhtml.png" alt="imagehtml">
                <img src="images/techcss.png" alt="imagecss">
                <img src="images/techjs.png" alt="imagejs">
                <img src="images/techjquery.png" alt="imagejquery">
                <img src="images/techbootstrap.png" alt="imagebootstrap">
            </div>
            <div class="feature">
                <p class="whatyou">What You will get ?...</p>
                <div class="featurepic">
                    <img src="images/featuremodern.svg" alt="featuremodern">
                    <div>
                        <h1 class="secondh1">...Modern design.</h1>
                        <p class="desc">there are many ways to create a web site: flat and material design, minimalist design, illustrations, vibrant colors, unique typography etc</p>
                    </div>
                </div>
                <div class="featurepic">
                    <img src="images/featuresmooth.svg" alt="featuresmoo">
                    <div>
                        <h1 class="secondh1">...Nice and smooth.</h1>
                        <p class="desc">pleasant scrolling, rich animation, hd visuals using CSS, JavaScript and JQuery</p>
                    </div>
                </div>
                <div class="featurepic">
                    <img src="images/featureadaptive.svg" alt="featureadaptive">
                    <div>
                        <h1 class="secondh1">...Adaptive design.</h1>
                        <p class="desc">all elements work to improve how content looks on a specific device</p>
                    </div>
                </div>
                <div class="featurepic">
                    <img src="images/featuremanage.svg" alt="featuremnage">
                    <div>
                        <h1 class="secondh1">...Easy to control.</h1>
                        <p class="desc">manage your web site easily</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="contacts" id="cont">
            <p class="pcont">CONTACT ME</p>                                 
            <div class="order">
                <form method="POST" action="php/form.php"><br>
                    <input type="text" name="fname" placeholder="Your Name:*"><br>               <!--pattern="[A-Za-z ]" required-->
                    <input type="email" name="femail" placeholder="E-mail:"><br>             <!--pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$"-->
                    <input type="tel" name="ftel" placeholder="Phone number:*"><br>            <!--pattern="[0-9]" maxlength="10" required-->
                    <textarea type="text" name="fcomment" rows="5" placeholder="write your comment..."></textarea>
                    <br><br>
                    <input type="submit" class="submit" value="I WANT A WEB SITE">
               </form>
            </div>
            <div class="feedback" id="feed">
                <form method="POST" action=""><br>              <!--ADD FEEDBACK PAGE OR FRAME-->
                    <textarea type="textarea" name="feedback" rows="5" placeholder="leave a feedback..."></textarea><br>          
                    <input type="submit" class="submit" value="LEAVE A FEEDBACK">
                </form>
            </div>
        </div>
        <div class="logo">
            <a href="https://www.facebook.com/gennadiy.aliev.1" target="_blank"><img src="images/logofacebook.svg" alt="logofacebook"></a>
            <a href="https://plus.google.com/u/0/108416854290997862687/posts" target="_blank"><img src="images/logogoogle.svg" alt="logogoogle"></a>
            <a href="skype:crysis_71?chat"><img src="images/logoskype.svg" alt="logoskype"></a>
            <div>
                <p class="logotext">&copy; 2016. Hennadii Aliiev. All Right Reserved</p>   <!--&copy; 2016-<?php echo date("Y");?>-->
            </div>
        </div>
    </div>  <!-- END WRAPPER-->
    <script src="script/script.js"></script>
</body>
</html>

   <!-- git checkout -b gh-pages 
     git add . 
     git commit -m "add new file"
     git push origin gh-pages -->